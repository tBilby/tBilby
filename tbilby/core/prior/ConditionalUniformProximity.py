...
class ConditionalUniformProximity(ConditionalBasePrior):
    ...
    def __init__(self, condition_func, name=None, latex_label=None, unit=None, boundary=None, **reference_params):
        ...
        boundary: str
            See superclass
        """
        super(ConditionalUniformProximity, self).__init__(condition_func,name=name, latex_label=latex_label, unit=unit,
                                                                    boundary=boundary, minimum=reference_params['minimum'], maximum=reference_params['maximum'])
        
        self.__class__.__name__ = 'ConditionalUniformProximity'
        self.__class__.__qualname__ = 'ConditionalUniformProximity'

        self._reference_params = reference_params
        self.width= reference_params['width']
        
        for key, value in reference_params.items():
            setattr(self, key, value)
       
    def set_n_fix_x(self,val=None):
        ...
        if hasattr(self,'x') and len(self.x) > 0 and val==None: # this is teh case for rescale which works well
    ...
    def rescale(self, val, **required_variables):
        ...
        samples=np.zeros(sz)
        #define inverse_cdf
        x_for_cdf = np.linspace(self.minimum, self.maximum, 10000).reshape(-1,1)            
        x_for_cdf = np.tile(x_for_cdf,(1,sz))
