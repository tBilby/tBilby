   ...
   # one must define the transdimensional_condition_function function, so we know what to do with conditional variables...  
    ...
    def transdimensional_condition_function(self,**required_variables):
        ...
        if(len(self.mu_l)>0): # handle the first mu case
            ...
            # assuming we know that the Lorentzian are located where the Gaussians, let's have a dynamic prior 
            
            # here you get access to the parameters in the following way:
            # self.mu[trans-dimensional component number ,sample_number]
            ...
            # len(mu_l), will give us the order of the lorentzian (i.e the number of component function we are dealing with) 
            ...
            if isinstance(self.n_gauss, float):
                ...
                n_g = self.n_gauss
            # since we know that the Gaussians are ordered (due to their conditional mean prior), we can set alpha and beta according to their values      
            N_component_function = self.mu_l.shape[0]
            
            beta = np.mean(self.mu[N_component_function,:])                
            alpha = np.mean(self.mu[N_component_function,:])                
            # skew the distrinution depending on the number of functions we are dealing with 
            if N_component_function >= 2: 
                alpha = np.mean(self.mu[N_component_function+2,:])/5                                
            if N_component_function < 2:     
                beta =  np.mean(self.mu[N_component_function+2,:])/5
...
component_functions_dict[gauss] = (n_peaks_g,'mu')
component_functions_dict[lorentzian] = (n_peaks_l,'mu_l')
...
if plot_result:
    ...
    result_processed,cols = tbilby.core.base.preprocess_results(result,component_functions_dict,remove_ghost_samples=False,return_samples_of_most_freq_component_function=True)
